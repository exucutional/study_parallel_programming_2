    ...
    #pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid, i)
    {
        ...
        #pragma omp sections nowait
        {
            #pragma omp section
            {
                ...
                omp_set_lock(&lockb);                           // Deadlock. Освободим locka до того как займем lockb
            ...
            #pragma omp section
            {
                ...
                for (i = 0; i < N; i++)
                    b[i] += PI * i;
                omp_unset_lock(&lockb);
                omp_set_lock(&locka);                           // Deadlock. Освободим lockb до того как займем locka. И уже после займем lockb
